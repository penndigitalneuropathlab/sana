...
#     class_preds = torch.cat([class_preds[:,0:1],class_preds[:,2:3],class_preds[:,3:4]],dim=1)
    ...
    for i in range(len(inputs)):
        ...
        # TODO: implement superimposed image of all class confidences
        # class_output = torch.empty((config['num_classes'],224,224))
        # # superimpose feature images
        # for class_idx in hip_cmap.keys():
        #     # Get 1xMxM feature image
        #     feature_img = class_preds[i][class_idx:class_idx+1].cpu().detach()

        #     # Add respective color to 1xMxM feat img, converting to 3xMxM RGB
        #     (r, g, b) = hip_cmap[class_idx]
        #     (r, g, b) = (r/255, g/255, b/255)
        #     feature_img = torch.cat([r*feature_img,g*feature_img,b*feature_img],dim=0)
        #     feature_img = torch.nn.functional.softmax(feature_img,dim=0)
            
        #     class_output = torch.cat([class_output,feature_img],dim=0)
        
        # class_output = torch.sum((class_output),dim=0)
        # class_output = class_output/config['num_classes']    
