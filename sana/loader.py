...
class Loader(openslide.OpenSlide):
    ...
    def load_frame(self, loc, size, level=None, pad_color=0):
        ...
        self.converter.to_pixels(loc, level)
        self.converter.to_pixels(size, level)
    ...
    def load_frame_with_segmentations(self, c1, c2, padding=None):
        """
        ...
        :param padding: amount of padding to apply to the Frame after rotation
        """
        c1 = c1.copy()
        c2 = c2.copy()
        ...
        # load the frame from the ROI, note that we've already performed the padding
    ...
    def from_vector(self, params, v, l, w, padding=0, logger=None):
        ...
        rect = Polygon(np.array([v[0][0]-w//2-padding//2, v[0][0]+w//2+padding//2, v[0][0]+w//2+padding//2, v[0][0]-w//2-padding//2]),
                       np.array([v[0][1]-padding//2, v[0][1]-padding//2, v[-1][1]+padding//2, v[-1][1]+padding//2]), False, self.level)
